...
dataset_list = {
    ...
    'cocokp': datasets.Coco,
    'cocodet': datasets.Coco,
    'eurocity': datasets.EuroCity,
    'nightowls': datasets.NightOwls
}
...
def cli():  # pylint: disable=too-many-statements,too-many-branches
    parser = argparse.ArgumentParser(
        prog='python3 -m openpifpaf.eval_test',
        description=__doc__,
    ...
    parser.add_argument('--dataset', type=str, default='cocokp',
    ...
    parser.add_argument('--skip-n', default=0, type=int,
                        help='skip n batches')
    parser.add_argument('--split', choices=('val', 'test', 'test-dev'), default='val',
    ...
    if args.split == 'val' and not args.detection_annotations:
        args.image_dir = IMAGE_DIR_VAL
        args.annotation_file = ANNOTATIONS_VAL
    elif args.split == 'val' and args.detection_annotations:
        args.image_dir = dataset.val_image_dir
        args.annotation_file = dataset.val_annotations
    elif args.split == 'test':
        args.image_dir = dataset.test_path['test-challenge']
        args.annotation_file = None
    elif args.split == 'test-dev':
        args.image_dir = dataset.test_path['test-dev-images']
        args.annotation_file = dataset.test_path['test-dev-annotations']
    else:
        raise Exception

    if args.split in ('test', 'test-dev') and not args.write_predictions and not args.debug:
        raise Exception('have to use --write-predictions for this dataset')
    if args.split in ('test', 'test-dev') and not args.all_images and not args.debug:
...
def write_evaluations(dataset, evalwrapper, filename, args, total_time, count_ops, file_size):
    if args.write_predictions:
        dataset.write_evaluations(evalwrapper, args.output, total_time)

    n_images = len(evalwrapper.image_ids)

    # if args.split not in ('test', 'test-dev'):
    #     #stats = evalwrapper.stats()
    #     np.savetxt(filename + '.txt', stats)
    #     with open(filename + '.stats.json', 'w') as f:
    #         json.dump({
    #             'stats': stats.tolist(),
    #             'n_images': n_images,
    #             'decoder_time': evalwrapper.decoder_time,
    #             'nn_time': evalwrapper.nn_time,
    ...
    print('decoder time = {:.1f}s ({:.0f}ms / image)'
          ''.format(evalwrapper.decoder_time, 1000 * evalwrapper.decoder_time / n_images))
...
def default_output_name(args):
    output = ('{}.eval'+args.dataset+'-{}edge{}').format(
        args.checkpoint,
        '{}-'.format(args.split) if args.split != 'val' else '',
        args.long_edge,
    )
    if args.n:
...
def main():
    ...
    evalwrapper = EvalWrapper(
        processor,
        max_per_image=500 if args.detection_annotations else 20,
        category_ids=[] if args.detection_annotations else [1],
    ...
    for batch_i, (image_tensors, anns_batch, meta_batch) in enumerate(data_loader):
        ...
        evalwrapper.decoder_time += processor.last_decoder_time
        evalwrapper.nn_time += processor.last_nn_time
        ...
        for pred, anns, meta in zip(pred_batch, anns_batch, meta_batch):
            evalwrapper.from_predictions(pred, meta, debug=args.debug, gt=anns)
    ...
    # write
    write_evaluations(data_loader.dataset, evalwrapper, args.output, args, total_time, count_ops, file_size)
