...
class Coco(torch.utils.data.Dataset):
    ...
    def __init__(self, image_dir, ann_file, *,
        ...
        self.category_ids2id = {categ:i+1 for i, categ in enumerate(self.category_ids)}

        self.ids = []
        for categ in self.category_ids:
            self.ids.extend(self.coco.getImgIds(catIds=[categ]))
    ...
    def __getitem__(self, index):
        ...
        anns_temp = []
        for ann in anns:
            ann['category_id'] = self.category_ids2id[ann['category_id']]
            anns_temp.append(ann)

        # preprocess image and annotations
        image, anns, meta = self.preprocess(image, anns_temp, meta)
