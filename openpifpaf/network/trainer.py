...
class Trainer():
    ...
    def train_batch(self, data, targets, apply_gradients=True):  # pylint: disable=method-hidden
        if self.device:
            ...
            for head in targets:
                if head is None:
                    targets_temp.append(None)
                elif isinstance(head, list):
                    targets_temp.append((head[0].to(self.device, non_blocking=True), head[1].to(self.device, non_blocking=True)))
                else:
                    targets_temp.append(head.to(self.device, non_blocking=True))
