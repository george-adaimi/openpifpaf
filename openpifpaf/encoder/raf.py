...
from .cifdet import CifDetGenerator
...
class RafGenerator:
    ...
    def __call__(self, image, anns, meta):
        ...
        detections = self.config.rescaler.relations(anns)
        bg_mask = mask = np.ones((
            (width_height_original[1] - 1) // self.config.stride + 1,
            (width_height_original[0] - 1) // self.config.stride + 1,
        ), dtype=np.bool)
        valid_area = self.config.rescaler.valid_area(meta)
        LOG.debug('valid area: %s', valid_area)

        self.init_fields(self.config.n_relations, bg_mask)
    ...
    def fill_keypoints(self, predicate, subject_det, object_det, other_subj, other_obj):
        ...
        scale1 = subject_det[2] * subject_det[3]
        scale2 = np.min([object_det[2], object_det[3]])
        ...
        max_r1 = CifDetGenerator.max_r(subject_det, other_subj)
        max_r2 = CifDetGenerator.max_r(object_det, other_obj)

        if self.config.sigmas is None:
            scale1, scale2 = scale1, scale2
        else:
            scale1 = scale1 * self.config.sigmas[joint1i]
            scale2 = scale2 * self.config.sigmas[joint2i]
