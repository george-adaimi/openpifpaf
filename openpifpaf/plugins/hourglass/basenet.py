...
class HeatmapLoss(torch.nn.Module):
    def __init__(self):
        super(HeatmapLoss, self).__init__()
        self.confidence_loss = openpifpaf.network.losses.components.Bce(detach_focal=True)
...
class HGNet_px2graph(nn.Module):
    nstack = 4
    def __init__(self, inp_dim, oup_dim,nstack, bn=False, increase=0,):
        super(HGNet_px2graph, self).__init__()
        self.nstack = nstack
        self.pre = nn.Sequential(
            Conv(3, 64, 7, 2, bn=True, relu=True),
            Residual(64, 256),
        ...
        self.features = nn.ModuleList( [
        nn.Sequential(
            Conv(inp_dim, inp_dim, 3, bn=True, relu=True)
        ) for i in range(self.nstack)])

        self.features_inter = nn.ModuleList( [
        nn.Sequential(
            Conv(inp_dim, inp_dim, 1, bn=True, relu=True)
        ) for i in range(self.nstack)])
        self.outs = nn.ModuleList( [Conv(inp_dim, oup_dim, 1, relu=False, bn=False) for i in range(nstack)] )
    ...
    def forward(self, x):
        ...
        for i in range(self.nstack):
            hg = self.hgs[i](x)
            feature = self.features[i](hg)
            combined_feats.append(self.features_inter[i](feature))
            if i < self.nstack - 1:
                preds = self.outs[i](self.relu(feature))
                combined_hm_preds.append(preds)
                #x = x + self.merge_preds[i](preds) + self.merge_features[i](feature)
...
class HGNet_base(openpifpaf.network.BaseNetwork):
    ...
    def __init__(self, name, inp_dim=512, oup_dim=170):
        model = HGNet_px2graph(inp_dim, oup_dim, nstack= self.nstack)
