...
from .annrescaler import AnnRescalerRel, mask_valid_area_offset
...
class RafGenerator:
    ...
    def __call__(self, image, anns, meta):
        ...
        if self.config.offset:
            self.visualizer.targets(fields[0], annotation_dicts=anns)
        else:
            self.visualizer.targets_offsets(fields, annotation_dicts=anns)
    ...
    def fields(self, valid_area, valid_area_offset):
        ...
        if self.config.offset:
            ...
            mask_valid_area_offset(fields_off1[0], valid_area_offset, fill_value=np.nan)
            mask_valid_area_offset(fields_off1[1], valid_area_offset, fill_value=np.nan)
            mask_valid_area_offset(fields_off2[0], valid_area_offset, fill_value=np.nan)
            mask_valid_area_offset(fields_off2[1], valid_area_offset, fill_value=np.nan)
